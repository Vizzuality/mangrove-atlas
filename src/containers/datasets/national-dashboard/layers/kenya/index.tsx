...
export function useSource(): SourceProps {
  return {
    id: 'national-dashboard-KENYA-source',
    type: 'vector',
    url: 'mapbox://globalmangrovewatch.3nw6ym2a',
  };
}
...
export function useLayers({ id, color }: { id: LayerProps['id']; color: string }): LayerProps[] {
  return [
    {
      id: `${id}-KENYA-layer`,
      source: 'national-dashboard-KENYA-source',
      'source-layer': 'MangroveExtent2020KenyaFinalQAv2',
      type: 'fill',
      paint: {
        'fill-color': color,
      },
...
const MangrovesNationalDashboardLayer = ({ beforeId, id }: LayerProps) => {
  ...
  const color = settings?.['national_dashboard_source_MangroveExtent2020KenyaFinalQAv2'];
