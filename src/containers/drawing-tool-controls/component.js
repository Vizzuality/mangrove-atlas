...
const DrawingToolControls = ({
  ...
  const handleChange = useCallback(() => myStorage.setItem('drawingAlert', false), [myStorage]);
    ...
    <div className={cx(styles.menuWrapper, { [styles.mobile]: mobile })}>
      {mobile ? (
        ...
        >
          ...
          />
          <span className={styles.menuItemTitle}>{isDrawingMobileMenu ? 'Place' : 'Custom'}</span>
        </button>
      ) : (
        <>
          ...
          <div className={styles.itemsWrapper}>
            ...
            >
              <Icon name="polyline" className={styles.icon} size="md" alt="create custom area" />
            </button>
          </div>
        </>
      )}
      <Modal isOpen={isOpenModalAlert} onRequestClose={handleCancel} closeButton centered>
        <div className={styles.modalContent}>
          <div className={styles.modalDescription}>
            ...
            <p>
              If you reset the page, <span>your custom area will be deleted</span>. Are you sure
              that you want to reset the page?
            </p>
            <div className={styles.modalCheckbox}>
              <label htmlFor="modal">Don’t ask me again.</label>
              <input type="checkbox" name="modal" id="modal" onChange={handleChange} />
            </div>
          </div>
