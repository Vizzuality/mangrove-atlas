...
function MangroveExtent({
  ...
  try {
    ...
    const quantity = unit === 'ha'
      ? numberFormat(coverage * 100)
    ...
    sentence = (
      <>
        ...
        <span>was </span>
        <strong className="notranslate">{numberFormat(chartData.data[0].area / 1000000)} </strong>{unitSelector}<span> in </span>{yearSelector},<span> this represents a linear coverage of <strong>{coveragePercentage}%</strong> </span> of the
